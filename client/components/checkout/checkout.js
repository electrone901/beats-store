...
function getStepContent(step, cart) {
  switch (step) {
    ...
    case 2:
      return (
        <Review addressVal={addressVal} paymentVal={paymentVal} cart={cart} />
      )
    default:
...
const Checkout = props => {
  ...
  addressVal = {firstName, lastName, address, city, stateAddress, country, zip}
  setAddressChange = {
    setFirstName,
    setLastName,
    setAddress,
...
const mapDispatch = dispatch => {
  return {
    completeOrder: (userId, targetObj) =>
      dispatch(completeOrder(userId, targetObj))
  }
}
